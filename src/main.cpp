...
void loop()
{
  ...
  if (spit_time > spit_min_time)
  { // Activate spit LED when spit time > 1s
  ...
  if (digitalRead(inFast) == LOW)
  { // Switch to fast mode (overrides normal operation)
#ifdef drain_system
    if (mist_pot_val == 0)
    {
  ...
  else if (mist_stat == LOW)
  { // Switch air mode or mist mode, depending on coolant output
    openAirValve();
    if (mist_pot_val > 1)
    { // If coolant > 1 ml/h, , execute spit mode and then switch to normal operation
      spitMode();
  ...
  { // Stops normal operation (including spit mode)
    stopStepper();
    spit_stat = 0;
  }
...
void spitMode()
{
  ...
  if (spit_stat == 0 && spit_time > spit_min_time)
  {
    ...
    {                              // Spit loop
      ...
      if (mist_stat == HIGH)
      { // Interrupt loop if mist mode has been stopped
        break;
      }
    }
